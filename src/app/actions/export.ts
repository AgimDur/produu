'use server'

import { cookies } from 'next/headers'
import { createServerComponentClient } from '@supabase/auth-helpers-nextjs'
import * as XLSX from 'exceljs'
import Papa from 'papaparse'

// Export products as CSV
export async function exportProductsAsCSV(): Promise<string> {
  const supabase = createServerComponentClient({ cookies })
  const { data: products, error } = await supabase
    .from('products')
    .select('*')
    .order('created_at', { ascending: false })

  if (error) {
    throw new Error(`Fehler beim Laden der Produkte: ${error.message}`)
  }

  const csvData = products?.map(product => ({
    Name: product.name,
    SKU: product.sku,
    'SKU-Level': product.sku_level,
    Kategorie: product.category,
    Preis: (product.price / 100).toFixed(2),
    Lagerbestand: product.stock,
    EAN13: product.ean13 || '',
    Beschreibung: product.description || '',
    'Parent ID': product.parent_id || '',
    'Erstellt am': new Date(product.created_at).toLocaleDateString('de-DE'),
    'Aktualisiert am': new Date(product.updated_at).toLocaleDateString('de-DE')
  })) || []

  return Papa.unparse(csvData)
}

// Export products as Excel
export async function exportProductsAsExcel(): Promise<Buffer | ArrayBuffer> {
  const supabase = createServerComponentClient({ cookies })
  const { data: products, error } = await supabase
    .from('products')
    .select('*')
    .order('created_at', { ascending: false })

  if (error) {
    throw new Error(`Fehler beim Laden der Produkte: ${error.message}`)
  }

  const workbook = new XLSX.Workbook()
  const worksheet = workbook.addWorksheet('Produkte')

  // Add headers
  worksheet.columns = [
    { header: 'Name', key: 'name', width: 20 },
    { header: 'SKU', key: 'sku', width: 15 },
    { header: 'SKU-Level', key: 'sku_level', width: 12 },
    { header: 'Kategorie', key: 'category', width: 15 },
    { header: 'Preis (€)', key: 'price', width: 12 },
    { header: 'Lagerbestand', key: 'stock', width: 12 },
    { header: 'EAN13', key: 'ean13', width: 15 },
    { header: 'Beschreibung', key: 'description', width: 30 },
    { header: 'Parent ID', key: 'parent_id', width: 15 },
    { header: 'Erstellt am', key: 'created_at', width: 15 },
    { header: 'Aktualisiert am', key: 'updated_at', width: 15 }
  ]

  // Add data
  products?.forEach(product => {
    worksheet.addRow({
      name: product.name,
      sku: product.sku,
      sku_level: product.sku_level,
      category: product.category,
      price: (product.price / 100).toFixed(2),
      stock: product.stock,
      ean13: product.ean13 || '',
      description: product.description || '',
      parent_id: product.parent_id || '',
      created_at: new Date(product.created_at).toLocaleDateString('de-DE'),
      updated_at: new Date(product.updated_at).toLocaleDateString('de-DE')
    })
  })

  // Style header row
  worksheet.getRow(1).font = { bold: true }
  worksheet.getRow(1).fill = {
    type: 'pattern',
    pattern: 'solid',
    fgColor: { argb: 'FFE0E0E0' }
  }

  return await workbook.xlsx.writeBuffer()
}

// Export orders as CSV
export async function exportOrdersAsCSV(): Promise<string> {
  const supabase = createServerComponentClient({ cookies })
  const { data: orders, error } = await supabase
    .from('orders')
    .select('*')
    .order('created_at', { ascending: false })

  if (error) {
    throw new Error(`Fehler beim Laden der Bestellungen: ${error.message}`)
  }

  const csvData = orders?.map(order => ({
    'Bestellnummer': order.order_number,
    'Kunde E-Mail': order.customer_email,
    'Kunde Vorname': order.customer_first_name || '',
    'Kunde Nachname': order.customer_last_name || '',
    'Gesamtpreis': (order.total_price / 100).toFixed(2),
    'Zwischensumme': (order.subtotal_price / 100).toFixed(2),
    'Steuern': (order.total_tax / 100).toFixed(2),
    'Währung': order.currency,
    'Zahlungsstatus': order.financial_status,
    'Versandstatus': order.fulfillment_status,
    'Bestellstatus': order.order_status,
    'Tags': order.tags || '',
    'Notiz': order.note || '',
    'Erstellt am': new Date(order.created_at).toLocaleDateString('de-DE'),
    'Verarbeitet am': order.processed_at ? new Date(order.processed_at).toLocaleDateString('de-DE') : '',
    'Storniert am': order.cancelled_at ? new Date(order.cancelled_at).toLocaleDateString('de-DE') : ''
  })) || []

  return Papa.unparse(csvData)
}

// Export orders as Excel
export async function exportOrdersAsExcel(): Promise<Buffer | ArrayBuffer> {
  const supabase = createServerComponentClient({ cookies })
  const { data: orders, error } = await supabase
    .from('orders')
    .select('*')
    .order('created_at', { ascending: false })

  if (error) {
    throw new Error(`Fehler beim Laden der Bestellungen: ${error.message}`)
  }

  const workbook = new XLSX.Workbook()
  const worksheet = workbook.addWorksheet('Bestellungen')

  // Add headers
  worksheet.columns = [
    { header: 'Bestellnummer', key: 'order_number', width: 15 },
    { header: 'Kunde E-Mail', key: 'customer_email', width: 25 },
    { header: 'Kunde Vorname', key: 'customer_first_name', width: 15 },
    { header: 'Kunde Nachname', key: 'customer_last_name', width: 15 },
    { header: 'Gesamtpreis (€)', key: 'total_price', width: 15 },
    { header: 'Zwischensumme (€)', key: 'subtotal_price', width: 15 },
    { header: 'Steuern (€)', key: 'total_tax', width: 12 },
    { header: 'Währung', key: 'currency', width: 10 },
    { header: 'Zahlungsstatus', key: 'financial_status', width: 12 },
    { header: 'Versandstatus', key: 'fulfillment_status', width: 12 },
    { header: 'Bestellstatus', key: 'order_status', width: 12 },
    { header: 'Tags', key: 'tags', width: 20 },
    { header: 'Notiz', key: 'note', width: 30 },
    { header: 'Erstellt am', key: 'created_at', width: 15 },
    { header: 'Verarbeitet am', key: 'processed_at', width: 15 },
    { header: 'Storniert am', key: 'cancelled_at', width: 15 }
  ]

  // Add data
  orders?.forEach(order => {
    worksheet.addRow({
      order_number: order.order_number,
      customer_email: order.customer_email,
      customer_first_name: order.customer_first_name || '',
      customer_last_name: order.customer_last_name || '',
      total_price: (order.total_price / 100).toFixed(2),
      subtotal_price: (order.subtotal_price / 100).toFixed(2),
      total_tax: (order.total_tax / 100).toFixed(2),
      currency: order.currency,
      financial_status: order.financial_status,
      fulfillment_status: order.fulfillment_status,
      order_status: order.order_status,
      tags: order.tags || '',
      note: order.note || '',
      created_at: new Date(order.created_at).toLocaleDateString('de-DE'),
      processed_at: order.processed_at ? new Date(order.processed_at).toLocaleDateString('de-DE') : '',
      cancelled_at: order.cancelled_at ? new Date(order.cancelled_at).toLocaleDateString('de-DE') : ''
    })
  })

  // Style header row
  worksheet.getRow(1).font = { bold: true }
  worksheet.getRow(1).fill = {
    type: 'pattern',
    pattern: 'solid',
    fgColor: { argb: 'FFE0E0E0' }
  }

  return await workbook.xlsx.writeBuffer()
} 